...
### Data
```{r}
ptitanic_train = read.csv("train.csv")
ptitanic_test = read.csv("test.csv")
str(ptitanic_train)
str(ptitanic_test)
```
...
```{r}
ptitanic_trn = subset(ptitanic_train, select = c(2, 3, 5, 6, 7, 8, 10, 12))
ptitanic_tst = subset(ptitanic_test, select = c(2, 4, 5, 6, 7, 9, 11))
colnames(ptitanic_tst)[1] = "Survived"
ptitanic_tst$Survived = 0
```

- Let's also change Survived column to a factor

```{r}
ptitanic_trn$Survived = as.factor(ptitanic_trn$Survived)
levels(ptitanic_trn$Survived) <- c("No", "Yes")

ptitanic_tst$Survived = as.factor(ptitanic_tst$Survived)
levels(ptitanic_tst$Survived) <- c("No", "Yes")
...
```{r}
apply(is.na(ptitanic_trn),2,sum)
```
...
```{r}
library(zoo)
ptitanic_trn$Age = round(na.aggregate(ptitanic_trn$Age))
```
